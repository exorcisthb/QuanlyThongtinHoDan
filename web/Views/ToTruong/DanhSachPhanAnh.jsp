...
<head>
    ...
    <title>Phản ánh - Tổ trưởng</title>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .stats-grid{display:grid;grid-template-columns:repeat(6,1fr);gap:12px;margin-bottom:26px;}
        .stat-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:16px 14px;display:flex;align-items:center;gap:12px;transition:border-color .18s;text-decoration:none;color:inherit;}
        ...
        .si-blue{background:rgba(79,142,247,.15);}.si-warn{background:rgba(251,191,36,.15);}.si-green{background:rgba(56,217,169,.15);}.si-red{background:rgba(247,92,92,.15);}.si-purple{background:rgba(167,139,250,.15);}.si-gray{background:rgba(100,116,139,.15);}
        ...
        .sv-blue{color:var(--accent);}.sv-warn{color:var(--warn);}.sv-green{color:var(--accent2);}.sv-red{color:var(--danger);}.sv-purple{color:#a78bfa;}.sv-gray{color:var(--muted);}
        .filter-bar{display:flex;align-items:center;gap:10px;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:14px 18px;margin-bottom:14px;}
        ...
        .tag{display:inline-flex;align-items:center;gap:3px;font-size:10px;font-weight:600;padding:2px 7px;border-radius:10px;margin-left:5px;vertical-align:middle;}
        .tag.cap{background:rgba(167,139,250,.12);color:#a78bfa;border:1px solid rgba(167,139,250,.3);}
        .tag.spam{background:rgba(247,92,92,.12);color:var(--danger);border:1px solid rgba(247,92,92,.3);}
        .act-wrap{position:relative;display:inline-flex;gap:6px;align-items:center;flex-wrap:wrap;justify-content:center;}
        ...
        .act-menu{position:fixed;background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:5px;min-width:220px;box-shadow:0 16px 48px rgba(0,0,0,.55);z-index:700;display:none;}
        ...
        .act-item.p:hover{background:rgba(167,139,250,.12);color:#a78bfa;}
        .act-item.r:hover{background:rgba(247,92,92,.12);color:var(--danger);}
        .act-item.o:hover{background:rgba(251,191,36,.12);color:var(--warn);}
        ...
        .btn-bad{background:var(--danger);color:#fff;}.btn-bad:hover{opacity:.88;}
        .btn-up{background:#a78bfa;color:#000;}.btn-up:hover{opacity:.88;}
        .btn-spam{background:var(--warn);color:#000;}.btn-spam:hover{opacity:.88;}
        ...
        /* ── Actions & inline form trong chi tiết ── */
...
<header class="topbar">
    <a href="${pageContext.request.contextPath}/totruong/dashboard" class="topbar-logo">
        ...
        <div class="logo-text">Tổ trưởng <span>Portal</span></div>
    </a>
    <div class="topbar-divider"></div>
    <nav class="topbar-nav">
        <a href="${pageContext.request.contextPath}/totruong/dashboard" class="tn-item">🏠 Tổng quan</a>
        <a href="${pageContext.request.contextPath}/totruong/phan-anh"  class="tn-item active">💬 Phản ánh</a>
        <a href="${pageContext.request.contextPath}/danh-sach-lich-hop" class="tn-item">📅 Lịch họp</a>
        <a href="${pageContext.request.contextPath}/todan/hodan"        class="tn-item">🏡 Hộ dân</a>
    </nav>
    <div class="topbar-spacer"></div>
    <div class="user-menu">
        <button class="avatar-btn" id="avatarBtn" onclick="toggleMenu()">
            <div class="avatar" id="avatarInitials">TT</div>
            <div><div class="avatar-name">${nguoiDung.ho} ${nguoiDung.ten}</div><div class="avatar-role">Tổ trưởng</div></div>
...
<div class="content">
    ...
    <div class="page-header">
        <div class="breadcrumb">Tổ trưởng <span>/ Quản lý Phản ánh</span></div>
        <h1>Danh sách Phản ánh</h1>
        <p>Tiếp nhận và xử lý phản ánh, kiến nghị từ người dân trong tổ</p>
    </div>

    <div class="stats-grid">
        <a href="?trangThai=1" class="stat-card"><div class="stat-icon si-blue">📥</div><div><div class="stat-val sv-blue">${soChoXuLy}</div><div class="stat-lbl">Chờ tiếp nhận</div></div></a>
        <a href="?trangThai=2" class="stat-card"><div class="stat-icon si-warn">⚙️</div><div><div class="stat-val sv-warn">${soDangXuLy}</div><div class="stat-lbl">Đang xử lý</div></div></a>
        <a href="?trangThai=4" class="stat-card"><div class="stat-icon si-green">✅</div><div><div class="stat-val sv-green">${soGiaiQuyet}</div><div class="stat-lbl">Đã giải quyết</div></div></a>
        <a href="?trangThai=3" class="stat-card"><div class="stat-icon si-purple">🔼</div><div><div class="stat-val sv-purple">${soChuyenCap}</div><div class="stat-lbl">Đã chuyển cấp</div></div></a>
        <a href="?trangThai=5" class="stat-card"><div class="stat-icon si-red">🚫</div><div><div class="stat-val sv-red">${soTuChoi}</div><div class="stat-lbl">Đã từ chối</div></div></a>
        <a href="?trangThai=6" class="stat-card"><div class="stat-icon si-gray">🗑</div><div><div class="stat-val sv-gray">${soSpam}</div><div class="stat-lbl">Spam</div></div></a>
    </div>

    <form action="${pageContext.request.contextPath}/totruong/phan-anh" method="get">
        <div class="filter-bar">
            <div class="search-wrap">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                <input type="text" name="keyword" value="${keyword}" placeholder="Tìm theo tiêu đề, người gửi...">
            </div>
            <select name="trangThai" class="sel">
                <option value="">Tất cả trạng thái</option>
                <option value="1" ${trangThaiFilter=='1'?'selected':''}>📥 Chờ tiếp nhận</option>
                <option value="2" ${trangThaiFilter=='2'?'selected':''}>⚙️ Đang xử lý</option>
                <option value="4" ${trangThaiFilter=='4'?'selected':''}>✅ Đã giải quyết</option>
                <option value="3" ${trangThaiFilter=='3'?'selected':''}>🔼 Đã chuyển cấp</option>
                <option value="5" ${trangThaiFilter=='5'?'selected':''}>🚫 Đã từ chối</option>
                <option value="6" ${trangThaiFilter=='6'?'selected':''}>🗑 Spam</option>
                <option value="7" ${trangThaiFilter=='7'?'selected':''}>❌ Đã hủy</option>
            </select>
            ...
            <c:if test="${not empty keyword or not empty trangThaiFilter or not empty mucDoFilter}">
                <a href="${pageContext.request.contextPath}/totruong/phan-anh" class="btn btn-secondary">✕ Xóa lọc</a>
            </c:if>
        </div>
    </form>

    <c:if test="${not empty trangThaiFilter or not empty mucDoFilter}">
        <div class="active-chips">
            <span class="chip-label">Đang lọc:</span>
            <c:if test="${not empty trangThaiFilter}">
                <span class="chip">Trạng thái:
                    <c:choose>
                        <c:when test="${trangThaiFilter=='1'}">Chờ tiếp nhận</c:when>
                        <c:when test="${trangThaiFilter=='2'}">Đang xử lý</c:when>
                        <c:when test="${trangThaiFilter=='3'}">Đã chuyển cấp</c:when>
                        <c:when test="${trangThaiFilter=='4'}">Đã giải quyết</c:when>
                        <c:when test="${trangThaiFilter=='5'}">Đã từ chối</c:when>
                        <c:when test="${trangThaiFilter=='6'}">Spam</c:when>
                        <c:otherwise>Đã hủy</c:otherwise>
                    </c:choose>
                    <a href="?keyword=${keyword}&mucDo=${mucDoFilter}">✕</a>
                </span>
            </c:if>
            <c:if test="${not empty mucDoFilter}">
                <span class="chip">Mức độ:
                    <c:choose>
                        <c:when test="${mucDoFilter=='1'}">Thấp</c:when>
                        <c:when test="${mucDoFilter=='2'}">Trung bình</c:when>
                        <c:otherwise>Cao</c:otherwise>
                    </c:choose>
                    <a href="?keyword=${keyword}&trangThai=${trangThaiFilter}">✕</a>
                </span>
            </c:if>
        </div>
    </c:if>
    ...
    <div class="table-wrap">
        ...
        <c:when test="${not empty danhSachPhanAnh}">
                    ...
                    <tr>
                        ...
                        <td style="max-width:260px">
                            <div style="font-weight:600;font-size:13px;line-height:1.4">
                                ${td}
                                <c:if test="${pa.daChuyenCap}"><span class="tag cap">🔼 Chuyển cấp</span></c:if>
                                <c:if test="${pa.isSpam}"><span class="tag spam">🗑 Spam</span></c:if>
                            </div>
                        ...
                        <td class="center">
                            <c:choose>
                                ...
                                <c:otherwise><span class="pill gray">${pa.tenMucDo}</span></c:otherwise>
                            </c:choose>
                        </td>
                        <td class="center">
                            <c:choose>
                                <c:when test="${ttID==1}"><span class="pill blue">📥 Chờ tiếp nhận</span></c:when>
                                <c:when test="${ttID==2}"><span class="pill warn">⚙️ Đang xử lý</span></c:when>
                                <c:when test="${ttID==3}"><span class="pill purple">🔼 Đã chuyển cấp</span></c:when>
                                <c:when test="${ttID==4}"><span class="pill ok">✅ Đã giải quyết</span></c:when>
                                <c:when test="${ttID==5}"><span class="pill err">🚫 Đã từ chối</span></c:when>
                                <c:when test="${ttID==6}"><span class="pill gray">🗑 Spam</span></c:when>
                                <c:when test="${ttID==7}"><span class="pill gray">❌ Đã hủy</span></c:when>
                                <c:otherwise><span class="pill gray">${pa.tenTrangThai}</span></c:otherwise>
                            </c:choose>
                        </td>
                        ...
                        <td class="center">
                            <div class="act-wrap">
                                ...
                                <button type="button" class="act-btn detail"
                                        ...
                                        data-chuyencap="${pa.daChuyenCap}"
                                        onclick="xemChiTiet(this)">
                                    🔍 Chi tiết
                                </button>

                                <%-- Nút thao tác nhanh bên ngoài --%>
                                <c:choose>
                                    <c:when test="${ttID==1}">
                                        <div style="position:relative;display:inline-block">
                                            <button type="button" class="act-btn" onclick="toggleAct('act-${paID}',event)">Thao tác ▾</button>
                                            <div class="act-menu" id="act-${paID}">
                                                <form action="${pageContext.request.contextPath}/totruong/phan-anh" method="post">
                                                    <input type="hidden" name="action" value="tiepNhan">
                                                    <input type="hidden" name="phanAnhID" value="${paID}">
                                                    <input type="hidden" name="keyword" value="${keyword}">
                                                    <input type="hidden" name="trangThai" value="${trangThaiFilter}">
                                                    <input type="hidden" name="mucDo" value="${mucDoFilter}">
                                                    <button type="submit" class="act-item b">📥 Tiếp nhận</button>
                                                </form>
                                                <hr class="act-sep">
                                                <button type="button" class="act-item p" onclick="openModal('chuyenCap','${paID}','${pa.tieuDe}')">🔼 Chuyển cấp</button>
                                                <button type="button" class="act-item r" onclick="openModal('tuChoi','${paID}','${pa.tieuDe}')">🚫 Từ chối</button>
                                                <button type="button" class="act-item o" onclick="openModal('spam','${paID}','${pa.tieuDe}')">🗑 Spam</button>
                                                <hr class="act-sep">
                                                <button type="button" class="act-item b" onclick="openModal('phanHoi','${paID}','${pa.tieuDe}')">💬 Gửi phản hồi</button>
                                            </div>
                                        </div>
                                    </c:when>
                                    <c:when test="${ttID==2}">
                                        <button type="button" class="act-btn"
                                                style="background:rgba(56,217,169,.1);border-color:rgba(56,217,169,.3);color:var(--accent2)"
                                                onclick="openModal('giaiQuyet','${paID}','${pa.tieuDe}')">✅ Giải quyết</button>
                                    </c:when>
                                    <c:when test="${ttID==3}">
                                        <span class="pill purple" style="font-size:11px">🔒 Chuyển cấp</span>
                                    </c:when>
                                </c:choose>
                            </div>
                        </td>
            ...
            <div class="table-footer">
                <span>Hiển thị <strong>${total}</strong> phản ánh
        ...
        <c:otherwise>
            <div class="empty-state">
                <div class="ei">💬</div>
                <p>${not empty keyword or not empty trangThaiFilter or not empty mucDoFilter ? 'Không tìm thấy phản ánh nào phù hợp.' : 'Chưa có phản ánh nào trong tổ của bạn.'}</p>
                <c:if test="${not empty keyword or not empty trangThaiFilter or not empty mucDoFilter}">
                    <a href="${pageContext.request.contextPath}/totruong/phan-anh" class="btn btn-secondary">← Xem tất cả</a>
                </c:if>
            </div>
        </c:otherwise>
        </c:choose>
        ...
        <div class="detail-scroll">
            ...
            <div class="inline-panel" id="inline-panel">
                <div class="inline-panel-inner">
                    <form id="inline-form" action="${pageContext.request.contextPath}/totruong/phan-anh" method="post">
...
<div class="modal-overlay" id="m-giaiQuyet" onclick="if(event.target===this)closeModal('giaiQuyet')">
    <div class="modal-box">
        ...
        <form action="${pageContext.request.contextPath}/totruong/phan-anh" method="post">
            ...
            <input type="hidden" name="keyword" value="${keyword}"><input type="hidden" name="trangThai" value="${trangThaiFilter}"><input type="hidden" name="mucDo" value="${mucDoFilter}">
            ...
            <div class="modal-foot"><button type="button" class="btn btn-secondary" onclick="closeModal('giaiQuyet')">Hủy</button><button type="submit" class="btn btn-ok">✅ Xác nhận</button></div>
        </form>
    </div>
</div>

<!-- ══ MODAL CHUYỂN CẤP ══ -->
<div class="modal-overlay" id="m-chuyenCap" onclick="if(event.target===this)closeModal('chuyenCap')">
    <div class="modal-box">
        <div class="modal-head"><div class="modal-icon">🔼</div><div class="modal-title">Chuyển cấp lên Cán bộ phường</div><div class="modal-sub" id="sub-chuyenCap"></div></div>
        <form action="${pageContext.request.contextPath}/totruong/phan-anh" method="post">
            <input type="hidden" name="action" value="chuyenCap"><input type="hidden" name="phanAnhID" id="id-chuyenCap">
            <input type="hidden" name="keyword" value="${keyword}"><input type="hidden" name="trangThai" value="${trangThaiFilter}"><input type="hidden" name="mucDo" value="${mucDoFilter}">
            <div class="modal-body"><label>Lý do chuyển cấp *</label><textarea name="ghiChu" placeholder="Nêu rõ lý do vượt quá thẩm quyền..." required></textarea></div>
            <div class="modal-foot"><button type="button" class="btn btn-secondary" onclick="closeModal('chuyenCap')">Hủy</button><button type="submit" class="btn btn-up">🔼 Xác nhận</button></div>
        </form>
    </div>
</div>

<!-- ══ MODAL TỪ CHỐI ══ -->
<div class="modal-overlay" id="m-tuChoi" onclick="if(event.target===this)closeModal('tuChoi')">
    <div class="modal-box">
        <div class="modal-head"><div class="modal-icon">🚫</div><div class="modal-title">Từ chối phản ánh</div><div class="modal-sub" id="sub-tuChoi"></div></div>
        <form action="${pageContext.request.contextPath}/totruong/phan-anh" method="post">
            <input type="hidden" name="action" value="tuChoi"><input type="hidden" name="phanAnhID" id="id-tuChoi">
            <input type="hidden" name="keyword" value="${keyword}"><input type="hidden" name="trangThai" value="${trangThaiFilter}"><input type="hidden" name="mucDo" value="${mucDoFilter}">
            <div class="modal-body"><label>Lý do từ chối *</label><textarea name="lyDo" placeholder="Nêu rõ lý do để người dân hiểu..." required></textarea></div>
            <div class="modal-foot"><button type="button" class="btn btn-secondary" onclick="closeModal('tuChoi')">Hủy</button><button type="submit" class="btn btn-bad">🚫 Xác nhận</button></div>
        </form>
    </div>
</div>

<!-- ══ MODAL SPAM ══ -->
<div class="modal-overlay" id="m-spam" onclick="if(event.target===this)closeModal('spam')">
    <div class="modal-box">
        <div class="modal-head"><div class="modal-icon">🗑</div><div class="modal-title">Đánh dấu Spam</div><div class="modal-sub" id="sub-spam"></div></div>
        <form action="${pageContext.request.contextPath}/totruong/phan-anh" method="post">
            <input type="hidden" name="action" value="danhDauSpam"><input type="hidden" name="phanAnhID" id="id-spam">
            <input type="hidden" name="keyword" value="${keyword}"><input type="hidden" name="trangThai" value="${trangThaiFilter}"><input type="hidden" name="mucDo" value="${mucDoFilter}">
            <div class="modal-body"><label>Lý do *</label><textarea name="ghiChu" placeholder="Vd: Nội dung không liên quan..." required></textarea></div>
            <div class="modal-foot"><button type="button" class="btn btn-secondary" onclick="closeModal('spam')">Hủy</button><button type="submit" class="btn btn-spam">🗑 Xác nhận</button></div>
        </form>
    </div>
</div>
...
<div class="modal-overlay" id="m-phanHoi" onclick="if(event.target===this)closeModal('phanHoi')">
    <div class="modal-box">
        <div class="modal-head"><div class="modal-icon">💬</div><div class="modal-title">Gửi phản hồi</div><div class="modal-sub" id="sub-phanHoi"></div></div>
        <form action="${pageContext.request.contextPath}/totruong/phan-anh" method="post">
            <input type="hidden" name="action" value="phanHoi"><input type="hidden" name="phanAnhID" id="id-phanHoi">
            <input type="hidden" name="keyword" value="${keyword}"><input type="hidden" name="trangThai" value="${trangThaiFilter}"><input type="hidden" name="mucDo" value="${mucDoFilter}">
            <div class="modal-body"><label>Nội dung phản hồi *</label><textarea name="noiDungPhanHoi" placeholder="Nhập nội dung gửi đến người dân..." required></textarea></div>
            <div class="modal-foot"><button type="button" class="btn btn-secondary" onclick="closeModal('phanHoi')">Hủy</button><button type="submit" class="btn btn-ok">💬 Gửi</button></div>
        </form>
    </div>
</div>
...
<div class="logout-overlay" id="logoutModal" onclick="if(event.target===this)hideLogoutModal()">
    <div class="logout-box">
        <div class="logout-body"><div class="logout-ico">🚪</div><div class="logout-title">Đăng xuất?</div><div class="logout-sub">Bạn sẽ được đưa về trang đăng nhập.</div></div>
...
const KW  = '${keyword}';
const TT_F = '${trangThaiFilter}';
const MD_F = '${mucDoFilter}';

(function(){
    const ten='${nguoiDung.ten}'||'TT';
    const s=ten.trim().split(' ').map(w=>w[0]).slice(-2).join('').toUpperCase()||'TT';
...
function toggleAct(id,e){
    e.stopPropagation();
    const menu=document.getElementById(id),isOpen=menu.classList.contains('open');
...
document.addEventListener('click',()=>document.querySelectorAll('.act-menu').forEach(m=>m.classList.remove('open')));
...
// ── Modal chi tiết ─────────────────────────────────────────────────
const PILL = {
    1:['blue','📥 Chờ tiếp nhận'],2:['warn','⚙️ Đang xử lý'],
    3:['purple','🔼 Đã chuyển cấp'],4:['ok','✅ Đã giải quyết'],
    5:['err','🚫 Đã từ chối'],6:['gray','🗑 Spam'],7:['gray','❌ Đã hủy']
};
const MUCD = {1:['ok','⬇ Thấp'],2:['warn','➡ Trung bình'],3:['err','⬆ Cao']};
const ACT_LABEL = {
...
function renderActions(ttID, id, title) {
    ...
    if (ttID == 1) {
        el.innerHTML =
            ...
            + '<form action="' + CTX + '/totruong/phan-anh" method="post" style="display:inline">'
            + '<input type="hidden" name="action" value="tiepNhan">'
            + '<input type="hidden" name="phanAnhID" value="' + id + '">'
            + '<input type="hidden" name="keyword" value="' + KW + '">'
            + '<input type="hidden" name="trangThai" value="' + TT_F + '">'
            + '<input type="hidden" name="mucDo" value="' + MD_F + '">'
            + '<button type="submit" class="btn" style="background:rgba(79,142,247,.15);color:var(--accent);border:1px solid rgba(79,142,247,.3)">📥 Tiếp nhận</button>'
            + '</form>'
            + '<button class="btn" style="background:rgba(167,139,250,.12);color:#a78bfa;border:1px solid rgba(167,139,250,.3)"'
            + ' onclick="showInlinePanel(\'chuyenCap\',\'' + id + '\',\'ghiChu\',\'Lý do chuyển cấp\',\'🔼 Xác nhận chuyển cấp\',\'btn-up\')">🔼 Chuyển cấp</button>'
            + '<button class="btn" style="background:rgba(247,92,92,.1);color:var(--danger);border:1px solid rgba(247,92,92,.3)"'
            + ' onclick="showInlinePanel(\'tuChoi\',\'' + id + '\',\'lyDo\',\'Lý do từ chối\',\'🚫 Xác nhận từ chối\',\'btn-bad\')">🚫 Từ chối</button>'
            + '<button class="btn" style="background:rgba(251,191,36,.1);color:var(--warn);border:1px solid rgba(251,191,36,.3)"'
            + ' onclick="showInlinePanel(\'danhDauSpam\',\'' + id + '\',\'ghiChu\',\'Lý do đánh dấu spam\',\'🗑 Xác nhận Spam\',\'btn-spam\')">🗑 Spam</button>'
            + '<button class="btn" style="background:rgba(56,217,169,.1);color:var(--accent2);border:1px solid rgba(56,217,169,.3)"'
        ...
        el.innerHTML =
            ...
            + ' onclick="showInlinePanel(\'tuChoi\',\'' + id + '\',\'lyDo\',\'Lý do từ chối\',\'🚫 Xác nhận từ chối\',\'btn-bad\')">🚫 Từ chối</button>'
            + '<button class="btn" style="background:rgba(56,217,169,.1);color:var(--accent2);border:1px solid rgba(56,217,169,.3)"'
            + ' onclick="showInlinePanel(\'phanHoi\',\'' + id + '\',\'noiDungPhanHoi\',\'Nội dung phản hồi\',\'💬 Gửi\',\'btn-ok\')">💬 Phản hồi</button>';
    } else {
