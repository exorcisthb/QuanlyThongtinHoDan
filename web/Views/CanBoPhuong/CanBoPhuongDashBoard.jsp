...
<html lang="vi">
    <head>
        ...
        <style>
            ...
            *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
            ...
            /* badge loại yêu cầu trong bảng tab 1 */
            .loai-badge{display:inline-block;font-size:9px;font-weight:700;padding:1px 7px;border-radius:10px;text-transform:uppercase;letter-spacing:.4px;margin-left:4px}
            .loai-ho{background:rgba(79,142,247,.15);color:var(--accent)}
            .loai-nk{background:rgba(168,85,247,.15);color:#c084fc}
        </style>
    </head>
    <body>
            ...
            <div class="content">
                ...
                <div class="cards-grid">
                    ...
                    <a href="${pageContext.request.contextPath}/canbophuong/phan-anh" class="dash-card red">
                        ...
                        <div class="dc-title">
                            Phản ánh chuyển cấp
                            <span class="dc-badge" id="statPhanAnh" style="display:none">0</span>
                        </div>
                        <div class="dc-desc">Tiếp nhận và giải quyết các phản ánh từ các tổ dân phố chuyển lên phường.</div>
                ...
                <div class="modal-body modal-body-list">
                    ...
                    <div class="ml-scroll">
                        ...
                        <table class="ml-table" id="mlTable" style="display:none">
                            <thead id="mlThead"><tr>
                                <th>Mã hộ / NK</th><th>Chủ hộ / Nhân khẩu</th><th>Tổ</th>
    ...
    (function loadPhanAnhBadge() {
        fetch(CTX + '/canbophuong/phan-anh?format=count', {credentials:'include'})
            .then(r => r.json())
            .then(function(d) {
                var so = d.soChuaXuLy || 0;
                var badge = document.getElementById('statPhanAnh');
                if (so > 0) { badge.textContent = so > 99 ? '99+' : so; badge.style.display = ''; }
            }).catch(function(){});
    })();

    function openModalTatCaThongBao() {
        openM('modalTatCaTB');
        if (_tbLoaded) { renderTbList(_tbFilter); return; }
        document.getElementById('tbList').innerHTML = '<div class="tb-empty"><div class="tb-empty-ico">⏳</div><p><span class="np-spinner"></span> Đang tải...</p></div>';
        fetch(CTX + '/thong-bao?action=list', {credentials:'include'})
            .then(r => r.json())
            .then(function(data) {
                _tbData = data.danhSach || []; _tbLoaded = true;
                var chuaDoc = data.chuaDoc || 0;
                document.getElementById('tbSub').textContent = _tbData.length + ' thông báo · ' + chuaDoc + ' chưa đọc';
                document.getElementById('tbMarkAllBtn').style.display = chuaDoc > 0 ? '' : 'none';
                renderTbList('all'); updateBadge(chuaDoc);
            }).catch(function(err) {
                document.getElementById('tbList').innerHTML = '<div class="tb-empty"><div class="tb-empty-ico">⚠️</div><p>Lỗi: ' + err.message + '</p></div>';
            });
    }

    function tbFilter(f, btn) { _tbFilter = f; document.querySelectorAll('.tb-ftab').forEach(b => b.classList.remove('active')); if(btn)btn.classList.add('active'); renderTbList(f); }

    function renderTbList(filter) {
        var list = filter==='unread' ? _tbData.filter(n=>!n.daDoc) : filter==='read' ? _tbData.filter(n=>n.daDoc) : _tbData;
        var el = document.getElementById('tbList');
        if (!list.length) { el.innerHTML = '<div class="tb-empty"><div class="tb-empty-ico">📭</div><p>' + (filter==='unread'?'Không có thông báo chưa đọc':'Không có thông báo nào') + '</p></div>'; return; }
        el.innerHTML = list.map(function(n) {
            var tag = detectTag(n.tieuDe), time = formatTime(n.ngayGui), unread = !n.daDoc;
            var preview = (n.noiDung||'').substring(0,100) + ((n.noiDung||'').length>100?'…':'');
            var sender = n.tenNguoiGui ? '· '+esc(n.tenNguoiGui) : '';
            return '<div class="tb-item '+(unread?'unread':'')+'" id="tbitem-'+n.thongBaoID+'" onclick="tbClickItem('+n.thongBaoID+','+unread+')">'
                + '<div class="tb-dot '+(unread?'':'read')+'" id="tbdot-'+n.thongBaoID+'"></div>'
                + '<div class="tb-body"><div class="tb-tag '+tag.cls+'">'+tag.label+'</div>'
                + '<div class="tb-title">'+esc(n.tieuDe)+'</div>'
                + (preview?'<div class="tb-preview">'+esc(preview)+'</div>':'')
                + '<div class="tb-meta"><span class="tb-time">'+time+'</span>'+(sender?'<span class="tb-sender">'+sender+'</span>':'')+'</div></div>'
    ...
    function tbClickItem(id, isUnread) {
        if (!isUnread) return;
        fetch(CTX+'/thong-bao', {method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({action:'doc',id}).toString(),credentials:'include'})
            .then(r=>r.json()).then(function() {
                var item=document.getElementById('tbitem-'+id), dot=document.getElementById('tbdot-'+id);
                if(item)item.classList.remove('unread'); if(dot)dot.classList.add('read');
                var entry=_tbData.find(n=>n.thongBaoID==id); if(entry)entry.daDoc=true;
                var remaining=_tbData.filter(n=>!n.daDoc).length;
                updateBadge(remaining);
                document.getElementById('tbSub').textContent=_tbData.length+' thông báo · '+remaining+' chưa đọc';
                if(remaining===0)document.getElementById('tbMarkAllBtn').style.display='none';
                var nitem=document.getElementById('nitem-'+id), ndot=document.getElementById('ndot-'+id);
                if(nitem)nitem.classList.remove('unread'); if(ndot)ndot.classList.add('read');
            }).catch(function(){});
    }

    function tbMarkAllRead() {
        fetch(CTX+'/thong-bao',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'action=docTatCa',credentials:'include'})
            .then(r=>r.json()).then(function(data) {
                if(data.success){_tbData.forEach(n=>n.daDoc=true);renderTbList(_tbFilter);updateBadge(0);document.getElementById('tbMarkAllBtn').style.display='none';document.getElementById('tbSub').textContent=_tbData.length+' thông báo · 0 chưa đọc';document.querySelectorAll('.np-item.unread').forEach(el=>el.classList.remove('unread'));document.querySelectorAll('.np-dot').forEach(d=>d.classList.add('read'));notifLoaded=false;showToast('success','✓ Đã đánh dấu tất cả đã đọc');}
            }).catch(function(){});
    }

    function showLogoutModal() { closeUserMenu(); document.getElementById('logoutModal').classList.add('show'); document.body.style.overflow='hidden'; }
    function hideLogoutModal() { document.getElementById('logoutModal').classList.remove('show'); document.body.style.overflow=''; }
    function closeUserMenu() { var dd=document.getElementById('userDropdown'),btn=document.getElementById('avatarBtn'); if(dd)dd.classList.remove('open'); if(btn)btn.classList.remove('open'); }

    function toggleNotif(e) { if(e)e.stopPropagation(); closeUserMenu(); var panel=document.getElementById('notifPanel'); var isOpen=panel.classList.toggle('open'); if(isOpen&&!notifLoaded)loadNotifs(); }

    function loadNotifs() {
        var listEl=document.getElementById('notifList');
        if(listEl)listEl.innerHTML='<div class="np-status"><span class="np-spinner"></span>Đang tải...</div>';
        fetch(CTX+'/thong-bao?action=list',{credentials:'include'})
            .then(r=>r.json()).then(function(data){notifLoaded=true;_tbData=data.danhSach||[];_tbLoaded=true;renderNotifs(data.danhSach||[]);updateBadge(data.chuaDoc||0);})
            .catch(function(err){if(listEl)listEl.innerHTML='<div class="np-status">⚠ Lỗi: '+err.message+'</div>';});
    }
    ...
    function renderNotifs(list) {
        ...
        el.innerHTML=list.slice(0,15).map(function(n){
            ...
            return '<div class="np-item '+(unread?'unread':'')+'" id="nitem-'+n.thongBaoID+'" onclick="clickNotif('+n.thongBaoID+','+unread+')">'
                +'<div class="np-dot '+(unread?'':'read')+'" id="ndot-'+n.thongBaoID+'"></div>'
                +'<div class="np-body"><div class="np-tag '+tag.cls+'">'+tag.label+'</div><div class="np-title">'+esc(n.tieuDe)+'</div>'
                +'<div class="np-noidung">'+esc(noiDung)+'</div>'
                +'<div class="np-time"><span>'+time+'</span>'+guiHtml+'</div></div><span class="np-arrow">→</span></div>';
        }).join('');
    }
    ...
    function openModalChiTiet(thongBaoID) {
        ...
        document.getElementById('myc_title').textContent='Chi tiết yêu cầu';document.getElementById('myc_sub').textContent='Đang tải...';
        document.getElementById('myc_skeleton').style.display='';document.getElementById('myc_content').style.display='none';
        document.getElementById('myc_footer').innerHTML='<button class="btn-m btn-cancel" onclick="closeM(\'modalYeuCau\')">Đóng</button>';
        openM('modalYeuCau');
    ...
    // FIX 1: helper lọc đúng theo tab
    // Tab 1 (Cư trú) = loại 1 + loại 3, Tab 2 (Thông tin NK) = loại 2
    function filterByType(data, type) {
        if (type === 1) return data.filter(r => r.loaiYeuCau === 1 || r.loaiYeuCau === 3);
        return data.filter(r => r.loaiYeuCau === 2);
    }

    // FIX 2: mlTypeFilter — dùng filterByType + đổi title tab 1
    function mlTypeFilter(type){
        ...
        else{ico.textContent='📋';ico.className='mico green';title.textContent='Yêu cầu đổi trạng thái cư trú & nhân khẩu';}
        updateTableHeader();
        var filteredByType=filterByType(_mlData,_mlType);
        renderMLSummary(filteredByType);renderMLTable('all');
    }

    // FIX 3: updateTableHeader — tab 1 dùng header chung hộ/NK
    function updateTableHeader(){
        ...
        if(_mlType===2){
            ...
            thead.innerHTML='<tr><th>Mã hộ / NK</th><th>Chủ hộ / Nhân khẩu</th><th>Tổ</th><th>Cũ → Mới</th><th>Ngày gửi</th><th>Trạng thái</th><th style="text-align:center">Thao tác</th></tr>';
        }
    }
    ...
    // FIX 4: renderMLTable — dùng filterByType, thêm render loại 3 trong tab 1
    function renderMLTable(filter){
        _mlFilter=filter;
        var byType=filterByType(_mlData,_mlType),filtered=filter==='all'?byType:byType.filter(r=>r.ttYC===filter);
        ...
        tbody.innerHTML=filtered.map(function(r){
            ...
            // Tab 2: loại 2 — cập nhật thông tin
            if(_mlType===2){
            ...
            // Tab 1: loại 3 — đổi trạng thái từng nhân khẩu
            if(r.loaiYeuCau===3){
                var tenNK3 = r.tenNhanKhau || '—';
                var maNK3  = r.maNhanKhau  || '—';
                var pillCu3=pillTT(r.trangThaiCuID,r.tenTrangThaiCu),pillMoi3=pillTT(r.trangThaiMoiID,r.tenTrangThaiMoi);
                return '<tr id="mlrow-'+r.yeuCauID+'">'
                    +'<td style="font-family:monospace;color:var(--accent);font-weight:600">'+esc(maNK3)+'<span class="loai-badge loai-nk">NK</span></td>'
                    +'<td><strong>'+esc(tenNK3)+'</strong></td>'
                    +'<td style="color:var(--muted)">'+esc(r.tenTo||'—')+'</td>'
                    +'<td>'+pillCu3+' → '+pillMoi3+'</td>'
                    +'<td style="color:var(--muted);font-size:12px">'+esc(r.ngayTao)+'</td>'
                    +'<td>'+ttBadge+'</td>'
                    +'<td style="text-align:center">'+actBtns+'</td></tr>';
            }

            // Tab 1: loại 1 — đổi trạng thái cả hộ
            var pillCu=pillTT(r.trangThaiCuID,r.tenTrangThaiCu),pillMoi=pillTT(r.trangThaiMoiID,r.tenTrangThaiMoi);
            return '<tr id="mlrow-'+r.yeuCauID+'">'
                +'<td style="font-family:monospace;color:var(--accent);font-weight:600">'+esc(r.maHoKhau)+'<span class="loai-badge loai-ho">Hộ</span></td>'
    ...
    function mlQuickTuChoi(yeuCauID){_tcYeuCauID=yeuCauID;document.getElementById('tcLyDo').value='';document.getElementById('tcErr').classList.remove('show');document.getElementById('tcLyDo').classList.remove('err');document.getElementById('modalTuChoi').classList.add('show');document.body.style.overflow='hidden';setTimeout(()=>document.getElementById('tcLyDo').focus(),100);}
    ...
    function renderModalChiTiet(d) {
        ...
        if (loai === 2) {
            ...
            document.getElementById('myc_title').textContent = 'Yêu cầu đổi trạng thái cư trú (nhân khẩu)';
            ...
            document.getElementById('myc_ico').textContent = '📋';
            document.getElementById('myc_ico').className   = 'mico warn';
            document.getElementById('myc_title').textContent = 'Yêu cầu đổi trạng thái cư trú (hộ)';
        ...
        if (loai === 3) {
            ...
            var fields = [
                ...
                {label:'CCCD',cu:d.cCCD_Cu,moi:d.cCCD_Moi},{label:'Quan hệ hộ',cu:d.quanHe_Cu,moi:d.quanHe_Moi},
                {label:'Dân tộc',cu:d.danToc_Cu,moi:d.danToc_Moi},{label:'Tôn giáo',cu:d.tonGiao_Cu,moi:d.tonGiao_Moi},
                {label:'Nghề nghiệp',cu:d.ngheNghiep_Cu,moi:d.ngheNghiep_Moi},
                {label:'Trình độ HV',cu:d.trinhDoHocVan_Cu,moi:d.trinhDoHocVan_Moi},
                {label:'Nơi sinh',cu:d.noiSinh_Cu,moi:d.noiSinh_Moi},{label:'Quê quán',cu:d.queQuan_Cu,moi:d.queQuan_Moi}
            ];
            var changedRows=fields.filter(f=>f.moi&&f.moi!=='null'&&f.moi!==null&&f.moi!==undefined).map(function(f){return'<tr><td style="color:var(--muted);font-size:12px;font-weight:600">'+esc(f.label)+'</td><td style="text-decoration:line-through;color:var(--muted);font-size:13px">'+orD(f.cu)+'</td><td style="color:var(--accent2);font-weight:600;font-size:13px">→ '+orD(f.moi)+'</td></tr>';}).join('');
            contentHtml+='<div class="sec-title">Thông tin muốn thay đổi</div><table class="nk-change-table"><thead><tr><th>Trường</th><th>Hiện tại</th><th>Muốn đổi thành</th></tr></thead><tbody>'+(changedRows||'<tr><td colspan="3" style="padding:20px;text-align:center;color:var(--muted)">⚠ Không có thông tin thay đổi</td></tr>')+'</tbody></table>'+formHtml;
        }
    ...
    function switchState(state){
        ...
        if(state==='duyet'){
            ...
            document.getElementById('myc_title').textContent=isNhanKhau?'Duyệt yêu cầu nhân khẩu':'Duyệt yêu cầu cư trú';
            ...
            document.getElementById('myc_title').textContent=isNhanKhau?'Từ chối yêu cầu nhân khẩu':'Từ chối yêu cầu cư trú';
        }else{
    ...
    function updateBadge(count){const b=document.getElementById('bellBadge');if(count>0){b.textContent=count>99?'99+':count;b.classList.remove('hidden');}else b.classList.add('hidden');document.getElementById('statChoDuyet').textContent=count>0?count:'0';}
    function pollBadge(){fetch(CTX+'/thong-bao?action=demChuaDoc',{credentials:'include'}).then(r=>r.json()).then(d=>updateBadge(d.chuaDoc||0)).catch(()=>{});}
    pollBadge(); setInterval(pollBadge,60000);
    function toggleMenu(){document.getElementById('notifPanel').classList.remove('open');const dd=document.getElementById('userDropdown'),btn=document.getElementById('avatarBtn');const open=dd.classList.toggle('open');btn.classList.toggle('open',open);}
    document.addEventListener('click',function(e){var bellWrap=document.getElementById('bellWrap'),userMenu=document.querySelector('.user-menu');if(bellWrap&&!bellWrap.contains(e.target))document.getElementById('notifPanel').classList.remove('open');if(userMenu&&!userMenu.contains(e.target))closeUserMenu();});
    ...
    // FIX 6: openModalYCList — dùng filterByType
    function openModalYCList(callback){
        ...
        document.getElementById('mlTitle').textContent='Yêu cầu đổi trạng thái cư trú & nhân khẩu';
        updateTableHeader();
        openM('modalYCList');
        if(_mlLoaded&&_mlData.length>0){
            var byType=filterByType(_mlData,_mlType);
        ...
        fetchYeuCauJSON().then(function(list){
            ...
            var byType=filterByType(list,_mlType);
            renderMLSummary(byType);renderMLTable('all');
            document.getElementById('mlLoading').style.display='none';
            document.getElementById('mlTable').style.display='';
            if(callback)callback();
        }).catch(function(err){document.getElementById('mlLoading').innerHTML='⚠ Không thể tải dữ liệu: '+err.message;});
    }
        </script>
    </body>
</html>
